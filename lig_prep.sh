...
if [ $(awk -F'/' '{print $NF}' <<<$lfile |  awk -F. '{print $NF}') != gro ]
 then
  ...
  then
   ...
   antechamber -i $lfile -fi mdl -o $name.mol2 -fo mol2 -c bcc -pf y -s 2 -nc $charge -rn $name || { >&2 echo "Failed to run command  at line ${LINENO} in bash script" && exit 1; }
  else
   cp $lfile $name.mol2
   >&2 echo 'Script running: Using prepared ligand *.mol2 file'
  fi
  parmchk2 -i $name.mol2 -f mol2 -o $name.frcmod || { >&2 echo "Failed to run command  at line ${LINENO} in bash script" && exit 1; }
  tleap -f tleap.in || { >&2 echo "Failed to run command  at line ${LINENO} in bash script" && exit 1; }
  python $script_path"/"pmed_amb2gmx.py -p $name.prmtop -x $name.inpcrd -o $name || { >&2 echo "Failed to run command  at line ${LINENO} in bash script" && exit 1; }
 else
#put into dir
...
# restraints
gmx genrestr -f $name.gro -o posre_$name.itp -fc 1000 1000 1000 <<< 2 || { >&2 echo "Failed to run command  at line ${LINENO} in bash script" && exit 1; }
