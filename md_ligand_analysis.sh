...
gmx rms -s md_out.tpr -f md_fit.xtc -o rmsd_$molid\.xvg -n index.ndx  -tu $tu <<< "Backbone  $index_ligand_noH" || { >&2 echo "Failed to run command  at line ${LINENO} of ${BASH_SOURCE}" && exit 1; }
