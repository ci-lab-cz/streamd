...
antechamber -fi gout -fo mol2 -i "${molid}_charg_gaussian.log" -o "_${molid}_tofix.mol2" -at "$ligand_forcefield" -c resp -j 5 -nc "$charge" -pf y -s 2 -dr no -rn "$resid" || { echo "Failed to run command  at line ${LINENO} in ${BASH_SOURCE}" && exit 1; }
...
antechamber -fi mol2 -fo mol2 -i "_${molid}_fixed.mol2" -o "$molid.mol2" -at "$ligand_forcefield" -nc "$charge" -pf y -s 2 -dr no -rn "$resid" -j 1 || { echo "Failed to run command  at line ${LINENO} in ${BASH_SOURCE}" && exit 1; }
