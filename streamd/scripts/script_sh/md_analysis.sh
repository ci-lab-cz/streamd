#!/bin/bash
#  args: wdir center_group trajectory_fit_group dtstep wdir_out_analysis
cd $wdir
...
printf '%s\n%s\n' "$center_group" "System" | gmx trjconv -s $tpr -f $deffnm\_noj_noPBC.xtc -o md_centermolsnoPBC.xtc -pbc mol -center -n index.ndx || { echo "Failed to run command  at line ${LINENO} of ${BASH_SOURCE}" && exit 1; }
# use it for PBSA https://github.com/Valdes-Tresanco-MS/gmx_MMPBSA/issues/33
printf '%s\n%s\n' "$trajectory_fit_group" "System" | gmx trjconv -s $tpr -f md_centermolsnoPBC.xtc -fit rot+trans -o md_fit.xtc -n index.ndx || { echo "Failed to run command  at line ${LINENO} of ${BASH_SOURCE}" && exit 1; }

printf '%s\n%s\n' "$trajectory_fit_group" "non-Water" | gmx trjconv -s $tpr -f md_centermolsnoPBC.xtc -fit rot+trans -o md_fit_nowater.xtc -n index.ndx || { echo "Failed to run command  at line ${LINENO} of ${BASH_SOURCE}" && exit 1; }
printf '%s\n%s\n' "$trajectory_fit_group" "non-Water" | gmx trjconv -s $tpr -f md_centermolsnoPBC.xtc -fit rot+trans -dump 0 -o md_out_nowater.gro -n index.ndx || { echo "Failed to run command  at line ${LINENO} of ${BASH_SOURCE}" && exit 1; }
